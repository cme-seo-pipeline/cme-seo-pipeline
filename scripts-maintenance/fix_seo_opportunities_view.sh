...
gsc AS (
  ...
  GROUP BY page, query, silo, sous_silo
),
ga4 AS (
...
SELECT
  ...
  -- CORRECTIF : plus de fausse valeur (0.7) quand la jointure GA4 echoue.
  -- NULL = honnete (pas de donnee), plutot qu'un chiffre invente.
  ...
  ROUND(
    ...
    -- CORRECTIF : neutre (x1) quand pas de donnee GA4, au lieu de supposer 0.5
...
FROM gsc g
LEFT JOIN ga4 a ON a.url=g.url
